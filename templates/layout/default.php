...
<body>
    <nav class="top-nav">
        <div class="top-nav-title">
            <a href="<?= $this->Url->build('/') ?>"><span>TP</span> Rodolphe</a>
        </div>
        <div class="top-nav-links">
                        <?= $this->Html->link(__('Accueil'), ['controller' => 'Pages', 'action' => 'display', 'home']) ?>
            <?= $this->Html->link(__('Gestion du Sommeil'), ['controller' => 'SleepRecords', 'action' => 'index']) ?>
            <?php if ($this->request->getAttribute('identity')): ?>
                <span><?= __('Hello, {0} {1}', h($this->request->getAttribute('identity')->prenom), h($this->request->getAttribute('identity')->nom)) ?></span>
                <?= $this->Form->postLink(__('Deconnexion'), ['controller' => 'Users', 'action' => 'logout'], ['confirm' => __('Are you sure you want to logout?')]) ?>
            <?php else: ?>
                <?= $this->Html->link(__('Connexion'), ['controller' => 'Users', 'action' => 'login']) ?>
            <?php endif; ?>
        </div>
    </nav>
    <main class="main">
